...
char read_shaft_encoder(void) {
	...
	static unsigned char old_value = 3;		// Step 2: Based on how you decided to mask PINC in step 1, what would new_value be if the switch is in a groove?
	
	// Function variables
	unsigned char new_value = PINC >> 6; // Step 1: Decide how to read PINC so that the push buttons are masked
	char rotation = 0;

	// If the knob was in a groove
	if (old_value == 3) {			// Step 3: Enter here what you did for step 2.
		if (new_value == 1)		// What will new_value be if you're rotating CW out of a groove
			rotation = 1;
		if (new_value == 2)		// What will new_value be if you're rotating CCW out of a groove
			rotation = -1;
	}
...
void  move_stepper_motor_by_step(int num_steps, int direction) {
	...
	static unsigned char stepPosition[] = {0x1F, 0x2F, 0x4F, 0x8F};
	static unsigned char position = 0;
	
	//Special case to prevent the motor being energized with no direction
	if ( !direction ) {
		return;
	}
	...
	int i = 0;
	for( ; i < num_steps; i++ ) {
		PORTE |= stepPosition[position]; //Set
		position = ((position + 4) + direction) % 4; //Determine new position
		wait_ms(2);
		PORTE &= 0x0F; //Clear
	}
